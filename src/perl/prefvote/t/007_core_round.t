...
throws_ok(sub {$core_round_ref->init_round_candidates()}, "PrefVote::Core::Round::PrevMissingException",
    "init_round_candidates() missing candidate data");

# test init_round_candidates() (4 tests)
my $core_round_ref2;
lives_ok(sub {$core_round_ref2 = PrefVote::Core::Round->new(number => 1,
    candidates => Readonly::Clone @candidate_names)}, "instantiate with candidates -> no exception");
lives_ok(sub {$core_round_ref2->init_round_candidates()}, "init_round_candidates() with candidates -> no exception");
...
foreach my $test (@set_result_tests) {
    my $round_ref;
    lives_ok(sub {$round_ref = PrefVote::Core::Round->new(number => 2, prev => $core_round_ref2)},
