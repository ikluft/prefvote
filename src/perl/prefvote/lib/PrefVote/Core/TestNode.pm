        ...
        if ( ref $value eq "Set::Tiny" ) {
            ...
            if ( not is_Int $opts{name} ) {
                PrefVote::Core::InternalDataException->throw(
                    classname   => __PACKAGE__,
                    attribute   => "name",
                    description => "array node node index is not numeric "
                        . $value->[ $opts{name} ] . " at "
                        . join( "-", $self->path() )
                );
            }
...
sub node_list
{
    ...
    if ( reftype $value ne "ARRAY" ) {
        ...
        PrefVote::Core::InternalDataException->throw(
            classname   => __PACKAGE__,
            attribute   => "value",
            description => "node_list expected a list value (got "
                . ( ( ref $value ) ? ref $value : "scalar" ) . ") at "
                . join( "-", $self->path() )
        );
    }
...
sub node_set
{
    ...
    if ( ref $value ne "Set::Tiny" ) {
        ...
        PrefVote::Core::InternalDataException->throw(
            classname   => __PACKAGE__,
            attribute   => "value",
            description => "node_set expected a set value (got "
                . ( ( ref $value ) ? ref $value : "scalar" ) . ") at "
                . join( "-", $self->path() )
        );
    }
