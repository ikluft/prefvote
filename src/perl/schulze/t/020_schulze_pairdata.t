...
is_deeply( $schulze_pairdata_ref->{forbidden},
    set(qw(test1 test2)), "after accessor forbidden(set of test1 test2): forbidden = set of test1 test2" );
is_deeply( $schulze_pairdata_ref->forbidden(),
    set(qw(test1 test2)), "accessor forbidden(set of test1 test2) returns set of test1 test2" );
dies_ok(
    sub { $schulze_pairdata_ref->forbidden( [ [qw(test1 test2)] ] ); },
    "accessor forbidden() dies as expected: given array where set wanted"
);
$schulze_pairdata_ref->path_history( [ [qw(test1 test2)] ] );
is_deeply(
    $schulze_pairdata_ref->{path_history},
    [ [qw(test1 test2)] ],
...
dies_ok(
    sub { $schulze_pairdata_ref->path_history( set(qw(test1 test2)) ); },
