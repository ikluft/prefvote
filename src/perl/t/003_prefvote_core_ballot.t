...
my $test_obj = PrefVote::Core::Ballot->new(items => \@foobar_choices);
...
# tests with fictitious A-F candidates (6 tests)
...
foreach my $test (@ballot_tests) {
    {
        ...
        my $ballot_obj = PrefVote::Core::Ballot->new(items => $test->{ballot});
