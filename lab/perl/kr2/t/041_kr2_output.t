#!/usr/bin/perl
# 041_kr2_output.t - tests for PrefVote::KR2::Output
use strict;
use warnings;
use autodie;
...
Readonly::Array my @result_expected => (
    ...
    {
        'rows' => [
            [ '', 'wins-loss', 'EVIL', 'FACTIOUS', 'CHAOTIC', 'BORING', 'ABNORMAL', 'DYSFUNCTIONAL' ],
            [
                'EVIL',
                undef,
                "\x{1f6c7}",
        ...
        'subtitle' => 'This compares how each choice ranks against others, ordered by Kluft algorithm.',
    },
);
...
# test fail-as-expected on empty command linea (2 tests)
{
    local @ARGV = ();
    my $vote_obj;
    lives_ok( sub { $vote_obj = PrefVote::KR2->instance(%core_params) },
        "1: instantiate PrefVote::KR2" );
...
{
    local @ARGV = qw(--format=RawCapture --method=kr2);
    my $vote_obj;
    lives_ok( sub { $vote_obj = PrefVote::KR2->instance(%core_params) },
        "2: instantiate PrefVote::KR2" );
    PrefVote::Core::Output::set_mock_stdin("");
...
# test with mock-stdin data (100 tests)
{
    ...
    #PrefVote::KR2::Output->debug_print("output = ".Dumper($output));
}
