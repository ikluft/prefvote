...
throws_ok(
    sub { $stv_round_ref->init_candidate_tally() },
...
lives_ok(
    sub {
        $stv_round_ref2 = PrefVote::STV::Round->new(
            number     => 1,
...
throws_ok(
    sub { $stv_round_ref2->add_votes_used(-10) },
    'PrefVote::STV::Round::NegativeIncrementException',
...
lives_ok(
    sub {
        $stv_round_ref2->sort_candidates( sub { return $_[1] cmp $_[0] } )
    },
...
lives_ok(
    sub {
        $stv_round_ref2->sort_candidates( sub { return $_[0] cmp $_[1] } )
    },
    "sort ascending -> no exception"
);
is_deeply( $stv_round_ref2->{candidates}, \@candidate_names, "candidates in original order" );
throws_ok(
    sub { $stv_round_ref2->sort_candidates(0) },
    'PrefVote::STV::Round::BadSortingFnException',
