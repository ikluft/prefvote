...
Readonly::Hash my %complex_checklist => ( tsut => { strset => [qw(a b)] }, );
Readonly::Array my @complex_value => ( tsut => PrefVote::Core::TestSpec::UnitTest->new( strset => set(qw(a b)) ), );
...
Readonly::Array my @tests => (
    ...
    {
        checklist   => {},
        check_value => PrefVote::Core::TestSpec::UnitTest->new(),
        result      => [],
        desc        => "checklist empty hash, value empty"
    },
    {
        checklist   => \%basic_checklist,
        check_value => PrefVote::Core::TestSpec::UnitTest->new(@basic_value),
        result      => \%basic_result,
        desc        => "basic test"
    },
    {
        checklist   => \%complex_checklist,
        check_value => PrefVote::Core::TestSpec::UnitTest->new(@complex_value),
...
is_deeply( PrefVote::Core::TestSpec->get_blackbox_spec('PrefVote::Core::TestSpec::UnitTest'),
    \%blackbox_spec, "get_blackbox_spec" );
...
foreach my $test (@tests) {
    ...
    if ( exists $test->{new_exception} ) {
        throws_ok(
            sub { $testspec_obj = PrefVote::Core::TestSpec->new( checklist => $test->{checklist} ) },
            $test->{new_exception},
            "exception expected (new): " . $test->{desc}
        );
        next;
    }
    ...
    if ( exists $test->{check_exception} ) {
        throws_ok(
            sub { $testspec_obj->check( $test->{check_value} ) },
            $test->{check_exception},
            "exception expected (check): " . $test->{desc}
        );
        next;
    }
    my @result;
    lives_ok(
        sub { @result = $testspec_obj->check( $test->{check_value} ) },
    ...
    foreach my $result_item (@result) {
        ...
        is_deeply(
            $result_item,
            $test->{result}{ $result_item->{description} },
