...
Readonly::Array my @ballot_tests => (
    ...
    { ballot => [qw(CTHULU)],                                              exception => "PrefVote::Core::Exception" },
    {
        ...
        exception => "PrefVote::Core::Exception"
    },    # this fails because allow_ties isn't set and it has a tie input
    {
        ballot    => [qw(EVIL FACTIOUS DYSFUNCTIONAL=CHAOTIC ABNORMAL BORING)],
        exception => "PrefVote::Core::Exception"
    },    # this fails because allow_ties isn't set and it has a tie input
...
sub ballot_tests
{
    ...
    foreach my $test (@ballot_tests) {
        ...
        if ( exists $test->{exception} ) {
            throws_ok( sub { $vote_obj->submit_ballot( array2ballot( $test->{ballot} ) ); },
                $test->{exception}, "ballot $summary_str -> exception $test->{exception} as expected" );
        } else {
            my $combo;
            ...
            if ( defined $combo ) {
                ...
                if ( exists $test->{total} ) {
                    is( $ballot_obj->items_count(),
                        $test->{total}, "ballot $summary_str has $test->{total} valid items" );
                }
            }
        }
    }
...
sub yaml_tests
{
    ...
    is_deeply(
        $vote_obj->testspec(),
        PrefVote::Core::TestSpec->new( checklist => $yaml_testspec ),
