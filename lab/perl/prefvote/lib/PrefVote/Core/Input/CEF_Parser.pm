...
#################
    ...
    sub _Parse
    {
        ...
        my ( $rules, $states, $lex, $error ) = @$self{ 'RULES', 'STATES', 'LEX', 'ERROR' };
        ...
        while (1) {
            ...
            if ( exists( $$actions{ACTIONS} ) ) {
                ...
                $act =
                      exists( $$actions{ACTIONS}{$$token} ) ? $$actions{ACTIONS}{$$token}
            ...
            defined($act)
                ...
                unpack( 'A1', $lhs ) eq '@'    #In line rule
                    and do {
                    $lhs =~ /^\@[0-9]+\-([0-9]+)$/
                        or die "In line rule name '$lhs' ill formed: " . "report it as a BUG.\n";
                    $$dotpos = $1;
                    };
...
sub _Error
{
    ...
    my $errmsg     = (
        ( exists $parser->YYData->{ERRMSG} ) ? $parser->YYData->{ERRMSG}
        : "Syntax error"
        )
        . " at position "
        . $parser->YYData->{CHARNO}
